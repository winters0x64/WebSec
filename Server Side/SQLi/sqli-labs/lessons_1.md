...
{While enumerating the number of columns in the original query it would be better to use 'union NULL,NULL --+, if there are two
columns as NULL could avoid the union restriction that the datatypes of the original columns and the columns that we inject should
be the same. } 
